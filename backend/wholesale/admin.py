...
class TrustBadgeInline(admin.TabularInline):
    model = WholesaleTrustBadge
    extra = 1
    fields = ["label", "order", "is_active"]
    verbose_name = "Trust Badge Pill"
    verbose_name_plural = "Trust Badge Pills (shown in 'Serving…' strip)"


class GuaranteeCheckInline(admin.TabularInline):
    model = WholesaleGuaranteeCheck
    extra = 1
    fields = ["text", "order"]
    verbose_name = "✓ Check Item"
    verbose_name_plural = "✓ Check Items (right side of bar)"
...
@admin.register(WholesaleHeroContent)
class WholesaleHeroContentAdmin(admin.ModelAdmin):
    list_display  = ["headline", "is_active", "image_preview", "updated_at"]
    list_editable = ["is_active"]
    readonly_fields = ["image_preview", "updated_at"]
    inlines = [TrustBadgeInline]

    fieldsets = (
        ("Copy", {
            "fields": ("headline", "headline_em", "subtitle", "trust_text"),
        }),
        ("Hero Image", {
        ...
        ("Floating Badges", {
            "fields": (
                ("badge_stat", "badge_label"),
                ("bottom_badge_title", "bottom_badge_subtitle"),
            ),
            "description": (
                "badge_stat / badge_label → top-right green box on the image.  "
                "bottom_badge_title / subtitle → white overlay card at the bottom."
            ),
        }),
        ("Visibility", {
...
@admin.register(WholesaleStat)
class WholesaleStatAdmin(admin.ModelAdmin):
    list_display  = ["value", "label", "sub", "order", "is_active"]
    list_editable = ["order", "is_active"]
    ordering = ["order"]
    fieldsets = (
        (None, {
            "fields": ("value", "label", "sub", "icon_svg", "order", "is_active"),
        }),
    )
...
@admin.register(WholesaleBenefit)
class WholesaleBenefitAdmin(admin.ModelAdmin):
    list_display  = ["title", "order", "is_active", "body_preview"]
    list_editable = ["order", "is_active"]
    ordering = ["order"]
...
class WholesaleCategoryAdmin(admin.ModelAdmin):
    list_display  = ["title", "badge", "badge_color_preview", "order", "is_active"]
    list_editable = ["order", "is_active"]
    ordering = ["order"]
    ...
    fieldsets = (
        ("Content", {
            "fields": ("title", "items", "icon_svg"),
        }),
        ("Badge", {
            "fields": ("badge", ("badge_bg_color", "badge_text_color")),
        }),
        ("Icon Style", {
    ...
    def badge_color_preview(self, obj):
        return format_html(
            ...
            '">{badge}</span>',
            bg=obj.badge_bg_color,
            fg=obj.badge_text_color,
            badge=obj.badge,
        )
    badge_color_preview.short_description = "Badge"
...
@admin.register(WholesaleStep)
class WholesaleStepAdmin(admin.ModelAdmin):
    list_display  = ["number", "title", "order", "is_active", "body_preview"]
    list_editable = ["order", "is_active"]
    ordering = ["order"]
...
@admin.register(WholesaleGuaranteeBar)
class WholesaleGuaranteeBarAdmin(admin.ModelAdmin):
    list_display  = ["title", "is_active", "updated_at"]
    list_editable = ["is_active"]
    readonly_fields = ["updated_at"]
    inlines = [GuaranteeCheckInline]

    fieldsets = (
        (None, {
            "fields": ("title", "subtitle", "is_active", "updated_at"),
            "description": (
                "This dark bar appears at the bottom of 'How It Works'.  "
                "Add ✓ check items via the inline section below."
            ),
        }),
    )