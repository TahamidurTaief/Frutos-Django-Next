...
class SectionItemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filter by section (accept both 'section' and 'section_id' params)
        section_id = (
            self.request.query_params.get('section_id') or
            self.request.query_params.get('section')
        )
        if section_id:
    ...
    def bulk_create(self, request):
        """
        Bulk create multiple section items in one request.
        Expects a JSON list: [ {section, product, ...}, {section, category, ...}, ... ]
        """
        items_data = request.data
        if not isinstance(items_data, list):
            return Response({'error': 'Expected a JSON array of items.'}, status=status.HTTP_400_BAD_REQUEST)
        if not items_data:
            return Response({'error': 'Empty list provided.'}, status=status.HTTP_400_BAD_REQUEST)

        serializers_list = [
            SectionItemCreateSerializer(data=item, context={'request': request})
            for item in items_data
        ]

        # Validate all first
        validation_errors = []
        for idx, ser in enumerate(serializers_list):
            if not ser.is_valid():
                validation_errors.append({'index': idx, 'errors': ser.errors})

        if validation_errors:
            return Response({'errors': validation_errors}, status=status.HTTP_400_BAD_REQUEST)

        # Save all
        created = [ser.save() for ser in serializers_list]
        return Response(
            SectionItemSerializer(created, many=True, context={'request': request}).data,
...
class PageSectionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filter by page (accept both 'page' and 'page_name' params)
        page_name = (
            self.request.query_params.get('page_name') or
