...
export default function WholesaleSupportTicketsTab({ accessToken }) {
  ...
  const fetchTickets = useCallback(async () => {
    try {
      setLoading(true)
      const res = await authFetch(`${API_BASE}/wholesale/tickets/`)
  ...
  useEffect(() => {
    if (selectedTicketId && tickets.length > 0) {
      ...
      if (!exists) {
        authFetch(`${API_BASE}/wholesale/tickets/${selectedTicketId}/`)
          .then(res => {
