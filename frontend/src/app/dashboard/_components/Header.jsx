  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden">
      ...
      {pending.length > 0 && (
        <div className="px-4 py-2.5 bg-amber-50 border-b border-amber-100">
          <div className="flex items-center gap-2">
            <ShoppingCart className="w-3.5 h-3.5 text-amber-600 shrink-0" />
      ...
      <div className="max-h-64 overflow-y-auto">
        {recent.length === 0 ? (
          ...
          recent.map((order) => {
            ...
            }[order.status?.toUpperCase()] || "bg-slate-100 text-slate-600";
            return (
              <div key={order.id || order.order_number} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
                <div className="p-1.5 bg-slate-100 rounded-lg shrink-0">
                  <Package className="w-3 h-3 text-slate-500" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-slate-800 truncate">#{order.order_number}</p>
                  <p className="text-[11px] text-slate-400 truncate">{order.customer_name} · ৳{Number(order.total_amount || 0).toLocaleString()}</p>
                </div>
                <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full capitalize shrink-0 ${statusColor}`}>
      ...
      <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50">
        <Link href="/dashboard/orders" onClick={onClose} className="flex items-center justify-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors">
