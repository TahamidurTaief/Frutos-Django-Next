...
function Input({ label, error, className = '', ...props }) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <input
        className={`bg-white dark:bg-gray-900 border ${error ? 'border-red-500' : 'border-gray-200 dark:border-gray-800'} rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all ${className}`}
        {...props}
      />
...
function Select({ label, error, children, className = '', ...props }) {
  return (
    <div className="flex flex-col gap-1">
      {label && <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{label}</label>}
      <select
        className={`bg-white dark:bg-gray-900 border ${error ? 'border-red-500' : 'border-gray-200 dark:border-gray-800'} rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all ${className}`}
        {...props}
      >
        {children}
      </select>
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div
        ...
        className="relative border-2 border-dashed border-gray-300 dark:border-gray-700 hover:border-blue-500/50 rounded-xl overflow-hidden cursor-pointer transition-all group bg-gray-50 dark:bg-gray-900/50"
      ...
      >
        {(preview || value) ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-xs text-gray-500 text-center px-4">Click to upload image</p>
          </div>
        )}
...
function PackFormModal({ pack, stores, storeSlug, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
      <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-2xl w-full max-w-xl max-h-[90vh] flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center">
              <Package size={17} className="text-blue-600 dark:text-blue-400" />
            </div>
            <div>
              <p className="font-bold text-gray-900 dark:text-white">{isEdit ? 'Edit Leftover Pack' : 'Add Leftover Pack'}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            {error && (
              <div className="flex items-center gap-2.5 p-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-lg text-red-600 dark:text-red-400 text-sm">
            ...
            <div className="flex flex-col gap-1">
                ...
                <textarea
                    className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition-all min-h-[80px]"
            ...
            <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
              ...
              />
              <label htmlFor="is_active" className="text-sm font-medium text-gray-900 dark:text-gray-200">
          ...
          <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex-shrink-0">
            <button type="button" onClick={onClose}
              className="px-5 py-2 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-900 dark:text-white text-sm font-semibold transition-colors">
              Cancel
            </button>
...
function DeleteConfirm({ pack, onClose, onConfirm, loading }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
      <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        <div className="w-12 h-12 rounded-xl bg-red-50 dark:bg-red-500/10 flex items-center justify-center mb-4">
          <Trash2 size={20} className="text-red-500 dark:text-red-400" />
        </div>
        <h3 className="font-bold text-gray-900 dark:text-white text-lg mb-1">Delete Pack</h3>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
          Are you sure you want to delete <span className="text-gray-900 dark:text-white font-semibold">{pack.name}</span>?
        </p>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-900 dark:text-white text-sm font-semibold transition-colors">
            Cancel
          </button>
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Leftover Packs</h1>
          <p className="text-sm text-gray-500 mt-1">Manage rescue packs, surplus stock, and daily leftovers.</p>
        </div>
        <button
          onClick={handleAdd}
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30"
          />
        </div>
        ...
        {stores.length > 1 && (
            <div className="flex items-center gap-2">
                ...
                <select
                    ...
                    className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                >
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-red-500">
            ...
            <button onClick={loadData} className="px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg text-sm text-gray-900 dark:text-white">Retry</button>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-sm font-semibold text-gray-600 dark:text-gray-300">No leftover packs found</p>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                {filteredPacks.map(pack => (
                  <tr key={pack.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {pack.image ? (
                          <img src={pack.image} alt={pack.name} className="w-10 h-10 rounded-lg object-cover border border-gray-200 dark:border-gray-700" />
                        ) : (
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500 truncate max-w-[200px]">{pack.description}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                        <div className="text-gray-900 dark:text-gray-300">{pack.package_type}</div>
                        {pack.weight_quantity && <div className="text-xs text-gray-500">{pack.weight_quantity}</div>}
                    </td>
                    <td className="px-6 py-4">
                        <div className="font-medium text-gray-900 dark:text-white">€{pack.price}</div>
                        {pack.original_price && pack.original_price > pack.price && (
                            <div className="text-xs text-gray-400 line-through">€{pack.original_price}</div>
                        )}
                    </td>
                    <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                            pack.stock > 0 ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'
                        }`}>
                            {pack.stock > 0 ? `${pack.stock} available` : 'Sold Out'}
                        </span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${pack.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                        {pack.is_active ? 'Active' : 'Hidden'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => handleEdit(pack)} className="p-1.5 text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-gray-800 rounded-md transition-colors">
                          <Edit2 size={16} />
                        </button>
                        <button onClick={() => setDeletePack(pack)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-gray-800 rounded-md transition-colors">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
