...
export function useModel(service, opts = {}) {
  ...
  const swrKey = useMemo(
    () => [service._basePath, JSON.stringify(params)],
    [service._basePath, params]
  );
  ...
  const totalCount = paginated ? (rawData?.count ?? data.length) : data.length;
  const nextPage = rawData?.next ?? null;
  const prevPage = rawData?.previous ?? null;
